...
class ClassificationModel:
    def __init__(self, text, morph=morph, mode="type"): # two modes: 'type' 'stop_dot'
        self.morph=morph
        self.text = text
        self.mode = mode
        if mode == 'type':
    ...
    def preprocess_and_lemmatize(self):
        """
        ...
        text = re.sub(r'[^а-яА-Я0-9\s]', '', self.text.lower())
        # Лемматизация слов
        lemmatized_words = [self.morph.parse(word)[0].normal_form for word in text.split()]
        return ' '.join(lemmatized_words)
    
    def count_matching_tokens(self):
        """
        Подсчитывает количество совпадений токенов из текста с ключевыми словами.
        """
        text_tokens = set(self.text.split())
        return sum(1 for word in self.keywords if word in text_tokens)
    
    def categorize_text(self, threshold=0.5):
        """
        ...
        tfidf_matrix = tfidf.fit_transform([self.text] + all_keywords)
    ...
    def get_class(self):
        ...
        try:
            category = self.categorize_text(threshold=0.07)[0]
        except:
