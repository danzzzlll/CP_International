...
def analytics_page():
    ...
    try:
        ...
        fig3 = px.line(day_count, x='created_at', y='Количество', title='Кол-во обращений в течение времени', color_discrete_sequence=color_discrete_sequence)
        fig3.update_xaxes(type='category')
        fig3.update_layout(xaxis_title='Дата', yaxis_title='Кол-во')
        st.plotly_chart(fig3)

        day_point_count = df_filtered.groupby([df_filtered['created_at'], 'Тип оборудования']).size().reset_index(name='Количество')
        fig4 = px.line(day_point_count, x='created_at', y='Количество', color='Тип оборудования', title='Кол-во обращений по типу обращения', color_discrete_sequence=color_discrete_sequence)
        fig4.update_xaxes(type='category')
        fig4.update_layout(xaxis_title='Дата', yaxis_title='Кол-во', legend_title='Тип оборудования')
        st.plotly_chart(fig4)

        plots = [fig2, fig1, fig3, fig4]
